...
class SpectatorList:
    """Class related to spectator list extraction.
    ...
    def getProcedures(self, year, accessName):
        """Main function. It will get the name, smiles and procedures of reactions with "accessName" pattern in the spectator list tag.
        ...
        variables = zip(spectator, name, smiles, procedure)
        patternProcedure = dict(zip(countlen, variables))

        df = Dataframe()
        data = df.createDataFrame(data=patternProcedure, columns=('spectator name', 'compound name', 'smiles', 'experimental procedure'), year=year)
        return data